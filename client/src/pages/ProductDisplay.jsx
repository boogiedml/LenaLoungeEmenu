  ...
  useEffect(() => {
    const fetchCat = async () => {
      try {
        ...
        const mealCategories = response?.data?.categoryList.filter(
          (c) => c.categoryType === "Meal"
        );
    ...
    const fetchPro = async () => {
      try {
        ...
        const mealProduct = response?.data?.productList.filter(
          (p) => p?.category?.categoryType === "Meal"
        );
        ...
        <div className="products_display">
          <div className="categories my-5 flex gap-5 overflow-scroll">
            {categories.map((c) => (
              ...
              >
                <div className="w-16 h-16 md:w-20 md:h-20 bg-myBrown lg:w-24 lg:h-24 mx-auto">
                  <img
                    src={c.image}
                    alt=""
