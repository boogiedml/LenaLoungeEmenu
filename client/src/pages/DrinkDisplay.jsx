  ...
  useEffect(() => {
    const fetchCat = async () => {
      try {
        ...
        const drinkCategories = response?.data?.categoryList.filter(
          (c) => c.categoryType === "Drink"
        );
    ...
    const fetchPro = async () => {
      try {
        ...
        const mealProduct = response?.data?.productList.filter(
          (p) => p?.category?.categoryType === "Drink"
        );
        ...
        <div className="products_display">
          <div className="categories my-5 flex gap-5 overflow-scroll">
            {categories.map((c) => (
              ...
              >
                <div className="w-16 h-16 md:w-20 md:h-20 bg-myBrown lg:w-[5.5rem] lg:h-[5.5rem] mx-auto">
                  <img
                    src={c.image}
                    alt=""
                ...
                <p className="mt-2 text-center text-xs font-inter uppercase line-clamp-2">
                  {c.name}
                </p>
              </motion.div>
            ))}
          </div>
          <h3 className="text-base lg:text-lg mb-3 mt-8 font-[500] uppercase text-black">
            {isActiveCategory}
          </h3>
          {isLoadingProducts ? (
            ...
            <div className="w-full flex-grow flex justify-center items-center text-center mt-4">
              No matching drink found.
            </div>
          ) : (
